    ...
    public class HomeController : Controller
    {
        ...
        public IActionResult Index()
        {
            HomeVM homeVM = new HomeVM
            {
             ...
             ExpensiveBooks = _db.Books.Include(x => x.BookImages).Include(x => x.Category).Include(x => x.Author).Include(x => x.BookTags).ThenInclude(x => x.Tag).OrderByDescending(x => x.SalePrice).Take(5).ToList()
            };
